...
metaphor migration generate Invoice telephony
metaphor migration run
...
(`telephony` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed telephony          # run Rust seeders
metaphor migration generate-seeds telephony  # emit SQL seed files
```
